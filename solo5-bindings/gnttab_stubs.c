...
/* Copied from MiniOS /gnttab.h */

#define NR_RESERVED_ENTRIES 8

/* NR_GRANT_FRAMES must be less than or equal to that configured in Xen */
#define NR_GRANT_FRAMES 4
#define NR_GRANT_ENTRIES (NR_GRANT_FRAMES * PAGE_SIZE / sizeof(uint64_t))
...
CAMLprim value
stub_gnttab_init(__attribute__((__unused__)) value unit)
{
    return Val_unit;
}
...
CAMLprim value
stub_gnttab_reserved(__attribute__((__unused__)) value unit)
{
    return Val_int(NR_RESERVED_ENTRIES);
}

CAMLprim value
stub_gnttab_nr_entries(__attribute__((__unused__)) value unit)
{
